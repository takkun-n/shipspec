...
async function generateWord(d) {
  ...
  children.push(
    ...
    new Table({width:{size:CW,type:WidthType.DXA}, columnWidths:[3746,2000,1500,2500], rows:[
      ...
      new TableRow({children:[lCell('1) 動力装置',3746), mkCell('ＡＣ ４４０Ｖ',{w:2000,center:true}), mkCell('60Hz',{w:1500,center:true}), mkCell('絶縁三相三線式',{w:2500,center:true})]}),
      new TableRow({children:[mkCell('',{w:3746}), mkCell('ＡＣ ２２０Ｖ',{w:2000,center:true}), mkCell('60Hz',{w:1500,center:true}), mkCell('絶縁三相三線式',{w:2500,center:true})]}),
      new TableRow({children:[lCell('2) 照明装置及び小型電気機器',3746), mkCell('ＡＣ １００Ｖ',{w:2000,center:true}), mkCell('60Hz',{w:1500,center:true}), mkCell('絶縁単相二線式',{w:2500,center:true})]}),
      new TableRow({children:[lCell('3) 通信及び航海計器装置',3746), mkCell('ＡＣ ４４０Ｖ',{w:2000,center:true}), mkCell('60Hz',{w:1500,center:true}), mkCell('絶縁三相三線式',{w:2500,center:true})]}),
      new TableRow({children:[mkCell('',{w:3746}), mkCell('ＡＣ １００Ｖ',{w:2000,center:true}), mkCell('60Hz',{w:1500,center:true}), mkCell('絶縁単相二線式',{w:2500,center:true})]}),
      new TableRow({children:[mkCell('',{w:3746}), mkCell('ＤＣ ２４Ｖ',{w:2000,center:true}), mkCell('－',{w:1500,center:true}), mkCell('絶縁二線式',{w:2500,center:true})]}),
    ]}),
    emptyP(),
    h2('2-2　使用電線'),
    ...
    para('1) 発電機　　　　　　　　　：0.6/1KV ＥＰゴム絶縁ビニルシースあじろがい装ケーブル（ＴＰＹＣ・ＭＰＹＣ・ＤＰＹＣ）'),
    para('2) 電動機・電熱器等　　　　：0.6/1KV ＥＰゴム絶縁ビニルシースあじろがい装ケーブル（ＴＰＹＣ）'),
    para('3) 照明灯・航海灯・蓄電池　：0.6/1KV ＥＰゴム絶縁ビニルシースあじろがい装ケーブル（ＴＰＹＣ）（ＤＰＹＣ）（ＤＰＹ）（ＴＰＹ）'),
    para('4) 通信機器　　　　　　　　：250V ＥＰゴム絶縁ビニルシースあじろがい装ケーブル（ＴＰＹＣ）（ＤＰＹＣ）（ＤＰＹ）（ＴＰＹ）'),
    para('5) 移動灯具　　　　　　　　：250V ＥＰゴム絶縁クロロプレンキャブタイヤコード（ＤＰＹ）'),
    para('6) 前部マスト・レーダーマストは、原則としてビニールがい装ケーブルを使用する。（ＤＰＹＣＹ）'),
    para('但し、メーカー支給ケーブルはメーカー標準とする。'),
    emptyP(),
    h2('2-3　電　路（配線工事）'),
    para('1. 一般に主要電路のケーブルは金属ハンガーで布設し、できる限り背後の金属構造物への塗装を妨げないように布設する。'),
    para('2. 居住区内張内木壁部に直接布設する。'),
    para('3. 暴露部の押さえバンドはＳＵＳとする。'),
    para('4. すべてのケーブルは金属性帯金又は結束バンドで固定する。'),
    para('5. ケーブルが水密隔壁又は甲板を貫通する場合には、電線貫通金物又は防水形電線貫通箱を使用する。'),
    para('6. ケーブルは、水・油又は温水管などの高温部から特に注意してできる限り離して布設し、機械的損傷を受け易い場所に布設する場合は、鋼板・鋼管又は防水形フレキシブルチューブを使用してケーブルを保護する。'),
    para('7. 船尾から船首への配線は甲板上に布設する。'),
    para('8. 電線の先端処理は十分に留意して行い、電線の接続及び分岐は接続箱又は端子箱などで行う。'),
    para('9. 接地工事は十分な施工をする。'),
    para('10. 各分電盤に予備のブレーカーを設備する。'),
    emptyP(),
  );
  ...
  children.push(
    ...
    new Table({width:{size:CW,type:WidthType.DXA}, columnWidths:[2746,1700,1500,1300,1300,1400], rows:[
      new TableRow({children:[hCell('用　途',2746),hCell('メーカー',1700),hCell('形　式',1500),hCell('電　圧',1300),hCell('容　量',1300),hCell('数　量',1400)]}),
      ...formRowsToTableRows(d.battRows, [2746,1700,1500,1300,1300,1400]),
    ]}),
    emptyP(),
    h2('3-3　変　圧　器'),
